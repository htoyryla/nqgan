...
def makeZ():
    noise = torch.FloatTensor(opt.batchSize, opt.nz, 1, 1).normal_(opt.mean,opt.std)
    return noise

#---------------generate images
def generate_img(model, epoch):
    ctr = 0
    ...
    for i in range(opt.howMany):
        ...
        for j in range (opt.steps):
          ...
          torchvision.utils.save_image(
                    ...
                    os.path.join(outdir, 'frame-%d_%d.png'%(int(epoch),ctr)),
                    nrow=1,
                    padding=0,
                    normalize=True)
          ctr = ctr + 1            
...
epochs = range(int(opt.which_epoch), int(opt.end_epoch) + 1, int(opt.step_epoch))
for epoch in epochs:
  print("*** "+str(epoch)+" ***")
  opt.which_epoch = str(epoch)+opt.suffix
  if opt.instance:
